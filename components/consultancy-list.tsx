...
export function ConsultancyList({ items, isCompact = false }: { items: ConsultancyProject[], isCompact?: boolean }) {
  const [selected, setSelected] = useState<ConsultancyProject | null>(null)
  ...
  return (
    <>
            ...
            {isCompact ? (
              <>
                <div>
                  ...
                  <h3 className="font-serif text-base sm:text-lg font-medium text-foreground mb-2 leading-snug group-hover:text-primary transition-colors">
                    {item.title}
                  </h3>
                </div>
                ...
                <div className="pt-3 border-t border-border/50 text-xs font-mono text-muted-foreground space-y-1">
                  ...
                  {item.role && <p><strong className="text-foreground">Role:</strong> {item.role}</p>}
      ...
      <BaseModal
        ...
        typeBadge={selected?.value ? <span className="font-mono text-sm font-semibold text-primary px-2.5 py-0.5 bg-primary/10">{selected.value}</span> : null}
      >
        {selected && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <p className="font-mono text-[0.7rem] uppercase tracking-widest text-muted-foreground mb-1">Client Organization</p>
                <p className="text-sm font-medium text-foreground">{selected.client}</p>
              </div>
              {selected.role && (
                <div>
                  <p className="font-mono text-[0.7rem] uppercase tracking-widest text-muted-foreground mb-1">Expert Role</p>
